...
class MonitorThread implements Runnable {
  ...
  void processCommand(String cmd) {
    ...
    if(cmd.equals("")) {
      ...
      throw new RuntimeException("Killed by input command. Main stack trace was:\n" + StrUtils.join(mainThread.getStackTrace(), "\n"));
    } else if (cmd.equals("dumpstack")) {
  ...
  public void run() {
    try {
      while(!stop) {
        ...
        if(killed) {
			final String stackTrace = StrUtils.join(mainThread.getStackTrace(), "\n");
			LogInfo.error("Killed by 'kill' file. Main stack trace was:\n" + stackTrace);
			throw new RuntimeException("Killed by 'kill' file. Main stack trace was:\n" + stackTrace);
		}
